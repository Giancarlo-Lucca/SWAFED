...
%TS: B: min y max, SS: Schweizer-Sklar,PP:producto y sumaprobabilistica

[fil,col,cap]=size(im);
%imF=padarray(im, [1 1], 'replicate');
Dil=zeros(fil,col,cap);
Ero=zeros(fil,col,cap);
...
for c=1:cap
    for i=1:fil
       for j=1:col
          ...
          matriz=matriz(:)';
    %Distintos  structuring element dependiendo de la posicion:
          se=[0.8600    0.8600    0.8600    0.8600    1.0000    0.8600    0.8600    0.8600    0.8600];
          if i==1
              if j==1
              se=[1 0.86 0.86  0.86];
          ...
          %Tomar en la erosion y dilation en lugar de min y max los de Schweizer-Sklar
          [fm,cm]=size(matriz);
          R=zeros(cm-1);
          R(1)=SS_S(X(1),X(2),-5);
          for z=2:cm-1
